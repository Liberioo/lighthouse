...
SELECT DISTINCT customer_id, SUM(total) as total_sales 
...
-- each customer's average ticket
SELECT DISTINCT customer_id, SUM(total)/COUNT(*) as avg_ticket 
  FROM lighthouse.orders o
  WHERE o.status = 'paid' OR o.status = 'confirmed'
  GROUP BY customer_id 
...
SELECT DISTINCT o.customer_id, COUNT(DISTINCT p.category_id) as categories
  FROM lighthouse.order_items oi
  JOIN lighthouse.orders o ON oi.order_id = o.id
